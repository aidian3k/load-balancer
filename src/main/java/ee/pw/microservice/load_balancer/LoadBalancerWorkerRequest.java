...
class LoadBalancerWorkerRequest extends Thread {
	...
	@SneakyThrows
	public void run() {
		String packageIdToFetchFromClientSocket = SocketHelpers.extractStringFromSocket(
			clientSocket
		);
		...
		String responseJsonObjectFromWorker = SocketHelpers.extractStringFromSocket(
			workerSocket
		);
		SocketHelpers.writeStringToSocket(
			responseJsonObjectFromWorker,
			clientSocket
		);
		logger.info(
